...
import { defaultLinks, defaultProfile } from '../data/linksData'

// 1. FETCH ALL LINKS FOR ADMIN (ACTIVE & INACTIVE)
...
export async function createLink(linkPayload) {
  if (!isSupabaseConfigured) {
    const newLink = {
      ...
      position: linkPayload.position || 99,
    }
    return newLink
  }
...
export async function updateProfile(profileUpdates) {
  if (!isSupabaseConfigured) return { ...defaultProfile, ...profileUpdates }
